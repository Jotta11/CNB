...
// ── Dimensões base do banner ─────────────────────────────────────────────────
// Mobile: aspect 5/8 (~750×1200px) | Desktop: 800px de altura (~1920×800px)
const BANNER_CLASS = 'mt-[72px] md:mt-20 aspect-[5/8] md:h-[800px] md:aspect-auto';
...
const HeroSkeleton = () => (
  <div
    id="inicio"
    className={`${BANNER_CLASS} relative overflow-hidden bg-primary/30 animate-pulse`}
  />
);

// ── Fallback textual (sem slides cadastrados ou erro) ────────────────────────
const HeroFallback = () => {
  ...
  return (
    <section
      id="inicio"
      className={`${BANNER_CLASS} flex items-center justify-center relative overflow-hidden`}
      style={{
        background:
    ...
    >
      ...
      <div className="container mx-auto px-4 text-center relative z-10">
        ...
        <p className="text-white/90 text-lg md:text-xl max-w-3xl mx-auto mb-12 leading-relaxed">
          Com a Conexão Norte Bovino, você compra e vende seu rebanho com
          inteligência de mercado e um processo comercial eficiente, seguro e
          profissional.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <button
            onClick={() => scrollTo('#lotes')}
            className="btn-hero-buy px-8 py-4 rounded-lg flex items-center gap-3 text-lg"
          >
          ...
          <button
            ...
            className="btn-hero-sell px-8 py-4 rounded-lg flex items-center gap-3 text-lg"
          >
  ...
  return (
    <div className="relative aspect-[5/8] md:h-[800px] md:aspect-auto w-full flex items-end md:items-center overflow-hidden">
      ...
      <div className="md:hidden relative z-10 w-full px-5 pb-8">
        ...
        {slide.botao_url && slide.botao_texto && (
          <button
        ...
        {count > 1 && <SlideDots count={count} current={current} />}
      </div>

      {/* Conteúdo desktop */}
      <div className="hidden md:flex relative z-10 flex-col items-start justify-center h-full px-20 max-w-2xl">
        ...
        {count > 1 && (
          <div className="mt-8">
            <SlideDots count={count} current={current} />
          </div>
        )}
      </div>
    </div>
...
// ── Carrossel principal ──────────────────────────────────────────────────────
const HeroCarousel = () => {
  const { slides, loading, isError } = useHeroSlides();
  ...
  // Mostra skeleton neutro enquanto carrega (sem o banner antigo)
  if (loading) return <HeroSkeleton />;

  // Fallback textual em caso de erro ou ausência de slides
  if (isError || !slides.length) return <HeroFallback />;

  // Slide único: sem controles de navegação
  if (slides.length === 1) {
    return (
